...
if [[ -n "$FILTER" ]]; then
  PROMPT="Load the e2e-runner skill. Run only tests matching '$FILTER'. Execute matching test files in test/e2e/ in sorted order. Follow the skill instructions exactly. Write the JSON report when done."
fi
